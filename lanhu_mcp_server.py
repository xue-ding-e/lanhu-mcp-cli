...
# 蓝湖用户生成内容(图名/文档名/页面名/备注等)是不可信外部输入,设计师可在其中注入
# __AI_INSTRUCTION__ / __SYSTEM__ 之类 prompt-injection 标记污染上层 AI。返回给上层前一律剥离。
_INJECTION_MARK_RE = re.compile(r'__\s*[A-Za-z][A-Za-z0-9_ ]{2,}\s*__')

def _sanitize_lanhu_text(s):
    """剥离蓝湖外部文本里的 prompt-injection 标记(__XXX__);非字符串原样返回。"""
    if not isinstance(s, str):
        return s
    return _INJECTION_MARK_RE.sub('', s).strip()

def _sanitize_deep(obj):
    """递归剥离 dict/list/str 里所有字符串的注入标记;在工具返回蓝湖数据前统一清理。
    注意:只清理来自蓝湖 API 的用户生成内容;不要用于 MCP 自身刻意构造的指令字段(本 fork 已无此类注入)。"""
    if isinstance(obj, str):
        return _sanitize_lanhu_text(obj)
...
def _format_css_value(key: str, value, factor: float = 1.0) -> str:
    """格式化CSS值。【rpx 统一(2026-07-03)】数值按 factor(=750/画布原始宽)归一 750 基准,
    单位标 rpx(与蓝湖 web 标注面板「微信小程序」单位一致);factor=1 且值已含 px 时也换算替换。"""
    if value is None:
        return ''
    ...
    if isinstance(value, (int, float)):
        if value == 0:
            return '0'
        return str(value) if key in _UNITLESS_PROPERTIES else f'{_n(value)}rpx'
    if isinstance(value, str):
        ...
        if re.match(r'^\d+(\.\d+)?$', value) and key not in _UNITLESS_PROPERTIES:
            return '0' if float(value) == 0 else f'{_n(float(value))}rpx'
        # 字符串里内嵌 px 数值(如 '14px'/'0px 4px'):逐个换算为 rpx
        if 'px' in value and key not in _UNITLESS_PROPERTIES:
            return re.sub(r'(-?[\d.]+)px\b', lambda m: f'{_n(float(m.group(1)))}rpx', value)
    return str(value)
...
def _generate_css(node: dict, css_rules: dict, loop_suffixes: list | None = None, factor: float = 1.0) -> None:
    """递归生成CSS规则。loop_suffixes 非空时，当前子树为循环模板，类名按 -0/-1/... 展开。
    factor: rpx 归一因子(=750/画布原始宽),数值经 _format_css_value 换算为 rpx。"""
    if not node:
        return
    ...
    if class_name:
        ...
        if style_entries or node.get('type') == 'lanhutext':
            ...
            for key, value in style_entries:
                ...
                css_value = _format_css_value(key, value, factor)
                if css_value:
    ...
    for child in children:
        _generate_css(child, css_rules, loop_suffixes, factor)
...
def convert_lanhu_to_html(json_data: dict) -> str:
    ...
    # 【rpx 统一(2026-07-03)】从根节点画布宽求归一因子(750/原始宽),schema 数值全部换算为 rpx 输出
    def _find_root_width(node, depth=0):
        if not isinstance(node, dict) or depth > 4:
            return 0
        w = (node.get('props', {}) or {}).get('style', {}).get('width')
        try:
            w = float(str(w).replace('px', '')) if w is not None else 0
        except (TypeError, ValueError):
            w = 0
        if w > 0:
            return w
        for child in (node.get('children') or []):
            cw = _find_root_width(child, depth + 1)
            if cw > 0:
                return cw
        return 0

    _root_w = _find_root_width(json_data)
    factor = (750.0 / _root_w) if _root_w else 1.0

    # 生成CSS
    _generate_css(json_data, css_rules, None, factor)
...
def convert_sketch_to_html(sketch_data: dict, design_scale: float = 2.0,
    ...
    """
    import math, re
    # 【rpx 统一(2026-07-03)】以画布原始宽归一到微信小程序 750rpx 基准,从原始值一步换算、只舍入一次。
    # 此前按 design_scale(@2x 假设)÷2:375px 图变 187.5 基准、单位标 px、二次换算放大舍入误差,
    # 与蓝湖 web 标注面板(金标准,小程序模式 rpx)对不上(实测 Rectangle 1039: web 618rpx vs 旧输出 154.5px)。
    raw_w = 0.0
    if 'artboard' in sketch_data:
        _af = sketch_data['artboard'].get('frame') or sketch_data['artboard'].get('realFrame') or {}
        raw_w = float(_af.get('width') or 0)
    elif 'board' in sketch_data:
        raw_w = float((sketch_data.get('board') or {}).get('width') or 0)
    factor = (750.0 / raw_w) if raw_w else (1.0 / (design_scale or 2.0))

    def px(v):
        if v is None:
            return 0
        r = round(float(v) * factor * 10) / 10
        # 整数值去 .0 尾巴(与蓝湖 web 面板显示一致,如 618 而非 618.0)
    ...
    def _gradient_css(grad):
        """蓝湖渐变 fill → CSS linear/radial-gradient(此前只取纯色,渐变元素背景/文字全丢)。"""
        if not isinstance(grad, dict):
            return None
        stops = grad.get('stops') or grad.get('colorStops') or []
        if len(stops) < 2:
            return None
        ...
        angle = round((math.degrees(math.atan2(dx, -dy)) + 360) % 360)
        parts = []
        for s in stops:
            c = s.get('color') or {}
            ...
            if not cv:
                continue
            pos = s.get('position', s.get('offset', 0))
            parts.append(f"{cv} {round(float(pos) * 100)}%")
        if len(parts) < 2:
            return None
        if grad.get('type') in (2, 'radial', 'Radial'):
            return f"radial-gradient({', '.join(parts)})"
        return f"linear-gradient({angle}deg, {', '.join(parts)})"

    def _element_gradient(L_, effects_):
        """从元素 fills 里找渐变(形状背景/文字渐变通用)。"""
        fills = (effects_.get('fills') if isinstance(effects_, dict) else None) or L_.get('fills') or []
        for f in fills:
            if isinstance(f, dict) and f.get('isEnabled', True) and str(f.get('type', '')).lower().find('gradient') >= 0:
                g = _gradient_css(f.get('gradient') or f)
                if g:
                    return g
        return None
    ...
    if 'artboard' in sketch_data:
        ...
        def _flatten(layer):
            ...
            # 组类型:保留组自身条目(带切图的组是可下载资源;不带的组保留框位置)后【始终递归内部】。
            # 旧逻辑两处丢层:① groupLayer 不在元组里→整组 append 一条、内部全丢(按钮容器
            # background/border-radius 就在组内,导致 1:1 复刻拿不到值);② 带切图的组不递归。
            if ltype in ('layerSection', 'symbolInstence', 'artboard', 'groupLayer'):
                images = layer.get('images') or {}
                # 容器自身若带视觉样式(填充/边框/切图),保留自身作背景层后再递归——
                # 否则 symbolInstence 按钮的蓝底/圆角(样式挂在容器自身而非子层)会整个丢失
                _eff = layer.get('layerEffects') or layer.get('style') or {}
                _has_visual = bool(
                    images.get('png_xxxhd') or images.get('svg')
                    or _eff.get('fills') or layer.get('fills') or _eff.get('borders')
                )
                if _has_visual:
                    layers.append(layer)
                for child in reversed(layer.get('layers', [])):
        ...
        def _flatten(layer):
            ...
            if w == 0 and h == 0:
                ...
                return
            ltype = layer.get('type', '')
            # 同 artboard 分支:组保留自身条目后【始终递归内部】,防 groupLayer/带切图组整组折叠丢层。
            if ltype in ('layerSection', 'symbolInstence', 'groupLayer'):
                images = layer.get('images') or {}
                # 同 artboard 分支:容器自身带视觉样式(填充/边框/切图)则保留自身作背景层,再递归内部
                _eff = layer.get('style') or {}
                _has_visual = bool(
                    images.get('png_xxxhd') or images.get('svg')
                    or (layer.get('fill') or {}).get('color')
                    or _eff.get('fills') or _eff.get('borders')
                )
                if _has_visual:
                    layers.append(layer)
                for child in reversed(layer.get('layers', [])):
    ...
    for idx, L in enumerate(layers):
        ...
        # 用 or 链与 _flatten 保持一致，防止 lframe.width=None/0 时兜底到 L 直属属性失效
        w = px(lframe.get('width') or L.get('width') or 0)
        h = px(lframe.get('height') or L.get('height') or 0)
        ...
        if ltype == 'textLayer' and (L.get('textInfo') or L.get('text')):
            ...
            if ti:
                ...
                # 补充 text-decoration（下划线/删除线）
                if ti.get('decoration'):
                    props.append(f"text-decoration:{ti.get('decoration')}")
                just = ti.get('justification', 'left')
                if just != 'left':
                    props.append(f"text-align:{just}")
                    annot['css']['text-align'] = just
                # 优先取 textInfo 原生 leading/lineHeight(精确行距),没有再回落 h/line_count 平均
                native_lh = ti.get('leading') or ti.get('lineHeight')
                lh_native = (px(native_lh.get('value', 0)) if isinstance(native_lh, dict) else px(native_lh)) if native_lh else 0
                if lh_native:
                    props.append(f"line-height:{lh_native}px")
                else:
            ...
            if fill_color:
                ...
                grad = _element_gradient(L, effects)
                if grad:
                    annot['css']['background'] = grad

        # 文本层渐变文字色(纯色 color 没提到时,如 VIP 金色渐变字):给完整 background-clip:text 写法
        if text_content and 'color' not in annot['css']:
            _tg = _element_gradient(L, effects)
            if _tg:
                annot['css']['background'] = _tg
                annot['css']['background-clip'] = 'text'
                annot['css']['-webkit-text-fill-color'] = 'transparent'

        css_rules.append(f".{cls}{{{';'.join(props)}}}")

        safe_name = (name or "").replace('"', '&quot;')
        # data-css/annotations 单位标 rpx(数值已 750 基准,微信小程序/uni-app x 直接抄);
        # 渲染 CSS(.elN)保留 px 供浏览器预览(750px 宽页面,数值相同)
        annot['css'] = {k: re.sub(r'(?<!r)px\b', 'rpx', str(v)) for k, v in annot['css'].items()}
...
(async () => {
  ...
  // rpx 统一:按画布原始宽归一 750 基准(此前硬 ÷2 只对 750 宽稿成立)
  const rawW = layers[0]?.width || 750;
  const factor = 750 / rawW;
  const dw = Math.round(rawW * factor * 10) / 10;
  const dh = Math.round((layers[0]?.height || 1334) * factor * 10) / 10;
  const px = v => Math.round(v * factor * 10) / 10;
...
def _extract_full_annotations_from_sketch(sketch_data: dict, design_scale: float = 2.0) -> str:
    ...
    # 【rpx 统一(2026-07-03)】同 convert_sketch_to_html:画布原始宽归一 750rpx,一步换算单次舍入
    _raw_w = float((sketch_data.get('board') or {}).get('width') or 0)
    if not _raw_w and 'artboard' in sketch_data:
        _af2 = sketch_data['artboard'].get('frame') or sketch_data['artboard'].get('realFrame') or {}
        _raw_w = float(_af2.get('width') or 0)
    _factor = (750.0 / _raw_w) if _raw_w else (1.0 / scale)
    ...
    def _px(val) -> str:
        """将设计稿原始值一步换算为 rpx(750 基准)字符串,单次舍入;整数去 .0 尾巴"""
        if val is None:
            return "0"
        r = round(float(val) * _factor, 1)
        return str(int(r)) if r == int(r) else str(r)
    ...
    def _extract_shadow_str(shadow_data: dict):
        ...
        return f"{color_str} {_px(x_off)}rpx {_px(y_off)}rpx {_px(blur)}rpx {_px(spread)}rpx"
    ...
    def _extract_stroke_str(frame_fx: dict):
        ...
        color_str = _rgba_str(color, op_val)
        return f"{_px(size)}rpx {pos} {color_str}"
    ...
    lines.append(f"画布尺寸: {_px(board_w)}x{_px(board_h)} rpx (750 基准)")
    lines.append(f"画布背景色: {board_color}")
    lines.append("")
    lines.append("以下所有尺寸/坐标单位均为 rpx(微信小程序/uni-app x 750 基准,与蓝湖 web 标注面板「微信小程序」单位一致,直接照抄)")
    lines.append("-" * 60)
    ...
    if text_layers:
        ...
        for t in text_layers:
            ...
            parts = []
            if t['fontSize']:
                parts.append(f"font-size: {t['fontSize']}rpx")
            if t['font']:
            ...
            if t['leading']:
                parts.append(f"line-height: {t['leading']}rpx")
            if t['tracking']:
...
class LanhuExtractor:
    ...
    def parse_url(self, url: str) -> dict:
        ...
        # 解析参数(URL 解码,防参数值含 %xx 编码被当字面量保存,破坏 pid/image_id)
        params = {}
    ...
    async def _download_file(self, url: str, local_path: Path):
        ...
        try:
            ...
            # 不崩(父调用方 gather return_exceptions=True),但记录失败便于排查(原静默 pass 会让缺资源难定位)
            print(f"[lanhu] 资源下载失败 {url}: {e}")
...
# Photoshop：蓝湖在根节点 type=ps，切图登记在 assets[]（isSlice），
    ...
    async def get_sketch_json(self, image_id: str, team_id: str = None, project_id: str = None) -> dict:
        ...
        if team_id:
            params["team_id"] = team_id
        response = await self.client.get(url, params=params)
        response.raise_for_status()  # 补 HTTP 状态码检查,4xx/5xx 提前报错而非把错误页解析成 JSON
        data = response.json()
        # 统一接受 0/'0'/'00000' 为成功(与 get_document_info 等口径一致)
        _code = data.get('code')
        if not (_code == 0 or _code == '0' or _code == '00000'):
            raise Exception(f"Failed to get design: {data['msg']}")
        result = data['result']
        latest_version = result['versions'][0]
        json_url = latest_version['json_url']
...
async def lanhu_list_product_documents(
    ...
    USE THIS WHEN 需列出项目下有哪些文档(列项目所有文档，不打开文档内容)。User says: 有哪些需求文档,
        列出项目的文档, 项目下的PRD列表, 产品文档列表,
    ...
    try:
        ...
        team_id = params.get('team_id')
        project_id = params.get('project_id')
        return _sanitize_deep(await extractor.list_product_documents(team_id, project_id))  # 递归剥离文档名等用户文本的注入标记
    finally:
        await extractor.close()
...
async def lanhu_get_pages(
    ...
    USE THIS WHEN 需打开某个具体文档读取其页面列表(取某文档页面，需已知 docId)。User says: 打开某文档,
        需求, PRD, 产品文档, 原型, 交互稿, Axure, 看看需求, 帮我看需求, 需求分析
...
"""
        ...
        # 【改造】四阶段工作流指引不再注入返回体(原 result['__AI_INSTRUCTION__'] 每次 ~5000 token,
        # 且用 __ 前缀伪装系统指令、与防注入冲突)。改为抽成独立工具 lanhu_get_workflow_guide(analysis_mode)
        # 按需取一次。下方 ai_suggestion 给轻量指引并指向该工具。
        # (注:本函数上方仍残留 ai_instruction_template 局部定义,已无引用,可后续清理;四阶段工作流已移至 lanhu_get_workflow_guide 工具。)
        _ = mode_options  # 占位符不再使用,避免 lint 未引用告警
        ...
        return _sanitize_deep(result)  # 返回前递归剥离蓝湖文本(文档名/页面名/文件夹名等)里的 __XXX__ 注入标记
    except httpx.HTTPStatusError as he:
        # 识别 401/403 登录态失效，给用户明确提示（参考 get_prototype_interactions 模式）
        _code = he.response.status_code if he.response is not None else 0
        if _code in (401, 403):
            return {'status': 'unauthorized', 'code': _code,
                    'message': '蓝湖登录态失效(LANHU_COOKIE 过期或无效),请更新 .env 的 LANHU_COOKIE'}
        raise
    finally:
        await extractor.close()
...
@mcp.tool()
async def lanhu_get_workflow_guide(
    analysis_mode: Annotated[str, "分析视角: 'developer'(开发·详细字段/规则/流程) / 'tester'(测试场景/校验) / 'explorer'(快速评审概览)。默认 developer"] = "developer",
    ctx: Context = None
) -> str:
    """
    [需求文档分析·工作流指引] 返回 Axure 需求文档/原型分析的【四阶段工作流】完整指引(按需取一次,不占每次调用 token)。

    配合 lanhu_get_pages / lanhu_get_ai_analyze_page_result 使用:那两个工具不再每次返回冗长工作流文本(原 __AI_INSTRUCTION__),
    需要详细 STAGE 1-4 流程 + 该模式的 STAGE2/STAGE4 输出格式时,调本工具取一次即可。

    USE THIS WHEN: 准备系统分析需求文档/PRD/原型,想要完整四阶段(全局扫描→分组深析→反向验证→生成交付物)工作流与输出格式。
    """
    mode_prompts = _get_analysis_mode_prompt(analysis_mode)
    parts = [
        "=" * 60,
        f"📋 Axure 需求文档分析 · 四阶段工作流(ZERO OMISSION) · 当前模式【{mode_prompts['mode_name']}】",
        f"   {mode_prompts['mode_desc']}",
        "=" * 60,
        "",
        "STAGE 1 全局文本扫描: lanhu_get_ai_analyze_page_result(page_names='all', mode='text_only')",
        "  → 建立结构上帝视角,用表格输出模块/页面/流程,再设计分组。完成后【必须】让用户选分析模式。",
        "STAGE 2 分组深析: 对每组 lanhu_get_ai_analyze_page_result(page_names=[该组], mode='full', analysis_mode=用户选的模式)",
        "  → 按下方该模式格式输出 + 标注变更类型(🆕新增/🔄修改/❓未明确)。",
        "STAGE 3 反向验证: 汇总所有结果,核对功能/字段/场景无遗漏,生成待确认清单。",
        "STAGE 4 生成交付物: 按下方该模式格式产出最终文档(不再调工具)。",
        "",
        "—— STAGE 2 输出格式(本模式)——",
        mode_prompts['stage2_prompt'],
        "",
        "—— STAGE 4 交付物格式(本模式)——",
        mode_prompts['stage4_prompt'],
        "=" * 60,
    ]
    return "\n".join(parts)
...
async def lanhu_get_ai_analyze_page_result(
    ...
    try:
        ...
        if is_text_only:
            ...
            # 【改造】STAGE 2/4 的详细输出格式 prompt 不再每次内联返回(原先每次几千 token);
            # 需要时调 lanhu_get_workflow_guide(analysis_mode) 取一次完整四阶段指引(含 STAGE2/STAGE4 格式)。
            header_text += "=" * 60 + "\n"
            header_text += f"📋 STAGE 2/4 输出格式: 调 lanhu_get_workflow_guide('{analysis_mode}') 取一次"
            header_text += f"（{mode_prompts['mode_name']}模式的完整四阶段指引）。\n"
            header_text += "=" * 60 + "\n\n"
        ...
        # 401/403 登录态失效给明确提示(参考 get_prototype_interactions)
        _code = he.response.status_code if he.response is not None else 0
        if _code in (401, 403):
            return [f'蓝湖登录态失效(LANHU_COOKIE 过期或无效),请更新 .env 的 LANHU_COOKIE (HTTP {_code})']
        raise
    finally:
        await extractor.close()
...
async def lanhu_get_designs(
    ...
    try:
        ...
        return _sanitize_deep(result)  # 递归剥离设计图名/分组名等用户文本的注入标记
    finally:
        await extractor.close()


@mcp.tool()
async def lanhu_get_prototype_interactions(
    url: Annotated[str, "Lanhu URL. 带 image_id(detailDetach/protoDetach 形式)时返回该图的交互;否则返回全项目交互图。Example: https://lanhuapp.com/web/#/item/project/protoDetach/edit?tid=xxx&pid=xxx&image_id=xxx (或 detailDetach?pid=xxx&image_id=xxx)。Required: pid。"],
    ctx: Context = None
) -> dict:
    """
    [交互原型/Interaction Prototype] 读取蓝湖【设计态原型(protoDetach)】的热区跳转关系——即"画在设计板上的交互"(点哪个区域跳到哪张图)。

    USE THIS WHEN user says: 交互, 交互原型, 原型交互, 热区, 跳转, protoDetach, 点哪跳哪, 交互图, 这页点了跳哪, 抽屉/弹窗怎么触发, 1:1还原交互, 交互流
    区别于 Axure 需求文档(那用 lanhu_get_pages / lanhu_get_ai_analyze_page_result)。本工具读的是设计师在设计稿上连的热区跳转,底层接口 GET /api/project/prototype。

    Returns:
        若 URL 带 image_id:
          - image_name: 该图名
          - hotspots: 本图各可点区域 [{rect:{x,y,w,h 归一化0-1}, target_id, target_name, animation}](按设计稿从上到下;target=点击跳转到的目标图)
          - opened_by: 哪些图的哪个区域跳转到/返回到本图 [{source_id, source_name, rect, animation}]
        否则返回全项目交互图 graph[]。
        坐标均为归一化 0-1(p_x/p_y/p_w/p_h),乘以图宽高得像素位置。
    """
    extractor = LanhuExtractor()
    try:
        params = extractor.parse_url(url)
        ...
        image_id = params.get('doc_id')  # detailDetach/protoDetach 的 image_id 解析后落在 doc_id
        api = f"{BASE_URL}/api/project/prototype?pid={pid}"
        if tid:
            api += f"&team_id={tid}"
        resp = await extractor.client.get(api)
        resp.raise_for_status()
        data = resp.json()
        result = data.get('result', data) if isinstance(data, dict) else {}
        images = result.get('images', []) if isinstance(result, dict) else []

        # id→图元信息:名字做注入过滤;width/height 实测 image 含有,供 AI 还原目标图尺寸/响应式
        id2info = {}
        for im in images:
            iid = im.get('id') or im.get('image') or im.get('image_id')
            if iid:
                id2info[iid] = {
                    'name': _sanitize_lanhu_text(im.get('name', '')),
                    'width': im.get('width'),
                    'height': im.get('height'),
                }

        def _name(iid):
            return id2info.get(iid, {}).get('name', '')

        def _r4(v):  # 归一化坐标截断 4 位小数(万分之一,1:1 还原足够;省 token)
            return round(v, 4) if isinstance(v, (int, float)) else v

        def _rect(ps: dict) -> dict:
            return {'x': _r4(ps.get('p_x')), 'y': _r4(ps.get('p_y')), 'w': _r4(ps.get('p_w')), 'h': _r4(ps.get('p_h'))}

        def _pack(ps: dict) -> dict:
            tgt = ps.get('next_image')
            info = id2info.get(tgt, {})
            out = {
                'rect': _rect(ps),
                'target_id': tgt,
                'target_name': info.get('name', ''),
            }
            if info.get('width') and info.get('height'):  # 目标图尺寸(供响应式/布局还原)
                out['target_w'] = info.get('width')
                out['target_h'] = info.get('height')
            if ps.get('animation_type'):  # 无转场(null/0)时省略,省 token
                out['animation'] = ps.get('animation_type')
            return out
        ...
        if image_id:
            ...
            for im in images:
                iid = im.get('id') or im.get('image') or im.get('image_id')
                positions = im.get('positions') or []
                if not isinstance(positions, list):  # API 返回格式异常时优雅降级,不崩
                    positions = []
                if iid == image_id:
                    found = True
                    hotspots = [_pack(p) for p in positions]
                for p in positions:
                    if p.get('next_image') == image_id:
                        ob = {
                            'source_id': iid,
                            'source_name': _name(iid),
                            'rect': _rect(p),
                        }
                        if p.get('animation_type'):
                            ob['animation'] = p.get('animation_type')
                        opened_by.append(ob)
            if not found:  # 区分"图不存在(image_id 错 / 不同 pid)"与"图存在但无交互"
                return {'status': 'not_found', 'image_id': image_id,
                        'message': f'图 {image_id} 不在本项目原型中(核对 image_id 与 pid 是否同一项目)'}
            # 按 y 排序(从上到下,贴合设计稿阅读顺序)
            hotspots.sort(key=lambda h: (h['rect']['y'] if h['rect']['y'] is not None else 0))
            return {
                'status': 'success',
                'image_id': image_id,
                'image_name': _name(image_id),
                'hotspots_count': len(hotspots),
                'hotspots': hotspots,
                'opened_by': opened_by,
                'note': 'rect 归一化0-1(乘图宽高得像素);target_w/h=目标图尺寸;hotspots=本图点击跳转目标;opened_by=哪些图跳来/返回本图。',
            }
        else:
            graph = []
            for im in images:
                ...
                if positions:
                    graph.append({
                        'image_id': iid,
                        'image_name': _name(iid),
                        'hotspots': [_pack(p) for p in positions],
                    })
            return {
                'status': 'success',
                'total_images_with_interactions': len(graph),
                'graph': graph,
                'note': '未指定 image_id,返回全项目交互图。rect 归一化0-1。',
            }
    except httpx.HTTPStatusError as he:
        ...
        if code in (401, 403):
            return {'status': 'unauthorized', 'code': code,
                    'message': '蓝湖登录态失效(LANHU_COOKIE 过期或无效),请更新 .env 的 LANHU_COOKIE'}
        return {'status': 'error', 'message': f'HTTP {code}: {he}'}
    except Exception as e:
...
async def lanhu_get_design_slices(
    ...
    try:
        ...
        # 5. [已弃用·不再返回] 旧 ai_workflow_guide 巨块(~170 行)+ 每 slice 10 个 scale_urls 曾把输出撑到 88KB
        #    超 token 限,被迫存盘+手写脚本解析。现已改为下方精简 return(只返 download_url/url_2x/url_3x)。
        #    本块为 dead code(构建后未返回),TODO: 可整块删除。下行 `_unused` 前缀标明不再使用。
        _unused_ai_workflow_guide = {
        ...
        # 精简输出(原 ai_workflow_guide ~170 行 + 每 slice 10 个 scale_urls 会撑爆 token,
        # 88KB 超限被迫存盘+手写脚本解析)。每 slice 只留 download_url(原图1x)+url_2x/url_3x(retina),
        # 去掉 iOS/Android 7 个同图变体与整段冗长引导。多倍率仍可按 OSS resize 自取(见 usage)。
        for s in (slices_data.get('slices') or []):
            su = s.pop('scale_urls', None)
        ...
        return {
            'status': 'success',
            **slices_data,
            'usage': '每 slice: download_url=原图(1x), url_2x/url_3x=retina。下载(Windows PowerShell): '
                     'Invoke-WebRequest -Headers @{Referer="https://lanhuapp.com/"} -Uri <url> -OutFile <path>。'
                     '文件名按 parent_name/name 取语义名。需其它像素尺寸:在 URL 追加 ?x-oss-process=image/resize,w_<宽>,h_<高>/format,png。',
        }
...
async def lanhu_say_list(
    ...
    limit: Annotated[Optional[int], "限制返回消息数量（防止上下文爆炸）。不传则不限制"] = None,  # 明确类型：整数或 None
    ctx: Context = None
) -> dict:
    """
    ...
    if not url or url.lower() == 'all':
        ...
        return _sanitize_deep(result)  # 递归剥离留言内容/作者名等用户文本的注入标记
...
async def lanhu_say_detail(
        message_ids: Annotated[Union[int, List[int]], "消息ID。单个数字或数组。例: 1 或 [1,2,3]"],  # 明确类型：单个 int 或 int 列表
